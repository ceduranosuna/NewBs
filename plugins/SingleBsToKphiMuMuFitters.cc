...
{//{{{
    ...
    while(fgets(lineBuff,512,fi) != NULL ){
        valBuff = strtok(lineBuff," ");
        if ( strcmp(valBuff,parName) == 0 ){            fprintf(fp,"%s",parName);
            int iVal = 0;
            while(iVal < nVal){
    ...
    printf("laaaaaaaaaaaaaaaaaaaaaalllllllllllllllaaaaaaaaaaaaaaaaaaaaaaaaaaa"); 
    fclose(fp);
    fclose(fi);
...
void bmass(int iBin=9 , const char outfile[] = "bmass")
{//{{{
  bool test = false; 
...
{//{{{
    ...
    //treein->Add("./data/2012/sel_BuTophiMuMu_NoGenFilter_8TeV_part*_mc.lite.root");
    //treein->Add("./data/2012/sel_BuTophiMuMu_NoGenFilter_8TeV_mc.lite_s*.root");
    //treein->Add("/Users/CeciliaD/Analysis/Bsphimumu/Nil/BTophiMuMu/ROOTFILES/unfiltmc-genOnly/sel_BTophiMuMu_NoFilt_8TeV_mc_genonly_s0.root");//yo agregue este archivo
    treein->Add("/Users/CeciliaD/Analysis/Bsphimumu/Nil/BTophiMuMu/ROOTFILES/mc/sel_BTophiMuMu_SCN_8TeV_normal_mc.lite_cut3-LHCb_s0.root");//tambien este
...
{//{{{
    ...
    //TChain *treein=new TChain("tree");
    //treein->Add("./data/2012/sel_BuTophiMuMu_NoGenFilter_8TeV_part*_mc.lite.root");//Contact po-hsun.chen@cern.ch to get these files.
    //treein->Add("/Users/CeciliaD/Analysis/Bsphimumu/Nil/BTophiMuMu/ROOTFILES/unfiltmc-genOnly/sel_BTophiMuMu_NoFilt_8TeV_mc_genonly_s0.root");
    TChain *treein=ch;
    if (treein == NULL) {
...
{//{{{
    ...
    RooRealVar fs("fs"   , "F_{S}"  , 0.01 , 0.  , 1.);//Derive from B0TophiJpsi , Bin3
    RooRealVar as("as"   , "A_{S}"  , -0.1 , -1. , 1.);//Derive from B0TophiJpsi , Bin3
    if (iBin != 3 && iBin != 5){
        // 2011 cross check
        fs.setVal(0.0129254);
        fs.setAsymError(-0.00898344,0.0101371);
...
int main(int argc, char** argv) {
    ...
    if (func == "bmass") {
        ...
        if (true) createAccptanceHist(); // For experts only. Set to true if no given acceptance_8TeV.root
        ch->Add(infile.Data());
        if (ch == NULL) gSystem->Exit(0);
        for (int iBin = 0; iBin < 8; iBin++) {
