...
// Class:      BToPhiMuMu
// 
/**\class BToPhiMuMu BToPhiMuMu.cc BphAna/BToPhiMuMu/src/BToPhiMuMu.cc
...
HistArgs hist_args[kHistNameSize] = {
  ...
  {"h_kstarmass", "Phi mass; M(K*) [GeV/^{2}]", 100, 0, 20},
...
class BToPhiMuMu : public edm::EDAnalyzer {
public:
  explicit BToPhiMuMu(const edm::ParameterSet&);
  ~BToPhiMuMu();
...
private:
  ...
  bool buildBuToPhiMuMu(const edm::Event &); 
  bool buildBdToPhiMuMu(const edm::Event &); 
  ...
  bool hasGoodKshortVertexMKC(const vector<reco::TrackRef>,
			      RefCountedKinematicTree &); 
  bool hasGoodPhiNeutralVertex(const reco::TransientTrack,
  ...
  bool hasGoodPhiChargedMass(RefCountedKinematicTree, double &); 
  ...
  bool isGenPhiCharged(const reco::Candidate *);
  ...
  void saveBuToPhiMuMu(RefCountedKinematicTree); 
  void saveBdToPhiMuMu(const RefCountedKinematicTree);
  ...
  bool BuildBuToPhiMuMu_; 
  bool BuildBdToPhiMuMu_; 
...
//
BToPhiMuMu::BToPhiMuMu(const edm::ParameterSet& iConfig):
  OutputFileName_(iConfig.getParameter<string>("OutputFileName")),
   
  BuildBuToPhiMuMu_(iConfig.getUntrackedParameter<bool>("BuildBuToPhiMuMu")),
  BuildBdToPhiMuMu_(iConfig.getUntrackedParameter<bool>("BuildBdToPhiMuMu")),
...
void
BToPhiMuMu::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    n_processed_ += 1; 
    ...
    if (KeepGENOnly_){
        ...
        if ( hasBeamSpot(iEvent) ) {
            ...
            if ( bFieldHandle_.isValid() && hasPrimaryVertex(iEvent) ) { 
                ...
                if (  ( BuildBuToPhiMuMu_ && buildBuToPhiMuMu(iEvent) ) ||
                      ( BuildBdToPhiMuMu_ && buildBdToPhiMuMu(iEvent) ) ) {  
...
BToPhiMuMu::beginJob()
{
  ...
  tree_ = new TTree ("tree", "BToPhiMuMu");
...
void 
BToPhiMuMu::beginRun(edm::Run const&, edm::EventSetup const&)
{
}

// method called when ending the processing of a run  ------------
void 
BToPhiMuMu::endRun(edm::Run const&, edm::EventSetup const&)
{
}

// method called when starting to processes a luminosity block  ------------
void 
BToPhiMuMu::beginLuminosityBlock(edm::LuminosityBlock const&, 
				   edm::EventSetup const&)
{
}
...
void 
BToPhiMuMu::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// method fills 'descriptions' with the allowed parameters for the module ---
void
BToPhiMuMu::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void
BToPhiMuMu::hltReport(const edm::Event& iEvent)
{
...
bool
BToPhiMuMu::hasBeamSpot(const edm::Event& iEvent)
{
...
bool
BToPhiMuMu::hasPrimaryVertex(const edm::Event& iEvent)
{
...
bool 
BToPhiMuMu::hasGoodTrack(const edm::Event& iEvent, 
...
bool 
BToPhiMuMu::buildBuToPhiMuMu(const edm::Event& iEvent)
{
  // init variables 
  ...
  for (vector<pat::Muon>::const_iterator iMuonM = patMuonHandle->begin(); 
	...
	for ( vector<pat::GenericParticle>::const_iterator iTrack
	  ...
	  passed = hasGoodPhiChargedMass(vertexFitTree, kstar_mass); 
...
bool 
BToPhiMuMu::buildBdToPhiMuMu(const edm::Event& iEvent)
{
  // init variables 
  edm::Handle< vector<pat::Muon> > patMuonHandle;
  iEvent.getByLabel(MuonLabel_, patMuonHandle);
  ...
  for (vector<pat::Muon>::const_iterator iMuonM = patMuonHandle->begin(); 
	...
	for ( vector<pat::GenericParticle>::const_iterator iTrackP
	  ...
	  if ( ! hasGoodPhiNeutralVertex(theTrackmTT, theTrackpTT, 
					   kstar_mass) ) continue; 
	  if ( kstar_mass < PhiMinMass_ || kstar_mass > PhiMaxMass_ ) continue;  

	  // check two tracks vertex for Bdbar 
	  if ( ! hasGoodPhiNeutralVertex(theTrackpTT, theTrackmTT, 
					   kstar_mass) ) continue; 
	  if ( kstar_mass < PhiMinMass_ || kstar_mass > PhiMaxMass_ ) continue;  
...
void 
BToPhiMuMu::calLS (double Vx, double Vy, double Vz,
...
void 
BToPhiMuMu::calCosAlpha (double Vx, double Vy, double Vz,
			   double Wx, double Wy, double Wz,
			   double VxErr2, double VyErr2, double VzErr2,
			   double VxyCov, double VxzCov, double VyzCov,
			   double WxErr2, double WyErr2, double WzErr2,
...
void
BToPhiMuMu::calCosAlpha2d (double Vx, double Vy, double Vz,
			     double Wx, double Wy, double Wz,
			     double VxErr2, double VyErr2, double VzErr2,
			     double VxyCov, double VxzCov, double VyzCov,
			     double WxErr2, double WyErr2, double WzErr2,
...
bool 
BToPhiMuMu::hasGoodMuonDcaBs (const reco::TransientTrack muTrackTT, 
...
bool 
BToPhiMuMu::hasGoodTrackDcaBs (const reco::TransientTrack TrackTT, 
...
bool 
BToPhiMuMu::hasGoodTrackDcaPoint (const reco::TransientTrack track, 
				    const GlobalPoint p, 
...
bool
BToPhiMuMu::calClosestApproachTracks (const reco::TransientTrack trackpTT, 
...
bool 
BToPhiMuMu::hasGoodMuMuVertex (const reco::TransientTrack muTrackpTT, 
...
bool
BToPhiMuMu::matchMuonTrack (const edm::Event& iEvent, 
...
bool
BToPhiMuMu::matchMuonTracks (const edm::Event& iEvent, 
...
bool 
BToPhiMuMu::hasGoodKshortVertex(const vector<reco::TrackRef> theDaughterTracks, 
...
bool 
BToPhiMuMu::hasGoodKshortVertexMKC(const vector<reco::TrackRef> theDaughterTracks, 
				     RefCountedKinematicTree &ksVertexFitTree)
{
...
bool 
BToPhiMuMu::hasGoodPhiNeutralVertex( const reco::TransientTrack pionTT, 
...
bool 
BToPhiMuMu::matchKshortTrack (const edm::Event& iEvent, 
				const reco::TrackRef theTrackRef)
{
  if ( theTrackRef.isNull() ) return false;
...
bool 
BToPhiMuMu::hasGoodPhiChargedMass(RefCountedKinematicTree vertexFitTree, 
				      double & kstar_mass)
{
  ...
  if ( kstar_mass < PhiMinMass_  || kstar_mass > PhiMaxMass_ ) 
    return false; 
...
bool 
BToPhiMuMu::hasGoodBuMass(RefCountedKinematicTree vertexFitTree, 
			    double & b_mass)
{
  vertexFitTree->movePointerToTheTop();
...
bool 
BToPhiMuMu::hasGoodBuVertex(const reco::TrackRef mu1Track,
...
bool 
BToPhiMuMu::hasGoodBdVertex(const reco::TransientTrack mu1TT,
...
void 
BToPhiMuMu::saveBuToPhiMuMu(RefCountedKinematicTree vertexFitTree){
...
void 
BToPhiMuMu::saveBdToPhiMuMu(const RefCountedKinematicTree vertexFitTree){
...
void 
BToPhiMuMu::saveBuVertex(RefCountedKinematicTree vertexFitTree){
  vertexFitTree->movePointerToTheTop();
  RefCountedKinematicVertex b_KV = vertexFitTree->currentDecayVertex();
...
void 
BToPhiMuMu::saveBuCosAlpha(RefCountedKinematicTree vertexFitTree)
{
...
void
BToPhiMuMu::saveBuCosAlpha2d(RefCountedKinematicTree vertexFitTree)
{
...
bool 
BToPhiMuMu::matchPrimaryVertexTracks () 
{
...
void 
BToPhiMuMu::saveBuLsig(RefCountedKinematicTree vertexFitTree)
{
  vertexFitTree->movePointerToTheTop();
  RefCountedKinematicVertex b_KV = vertexFitTree->currentDecayVertex();
  double LSBS, LSBSErr; 
...
void 
BToPhiMuMu::calCtau(RefCountedKinematicTree vertexFitTree, 
...
double 
BToPhiMuMu::calEta (double Px, double Py, double Pz)
{
...
double 
BToPhiMuMu::calPhi (double Px, double Py, double Pz)
{
...
double
BToPhiMuMu::calEtaPhiDistance (double Px1, double Py1, double Pz1,
...
void 
BToPhiMuMu::saveBuCtau(RefCountedKinematicTree vertexFitTree)
{
...
void
BToPhiMuMu::saveGenInfo(const edm::Event& iEvent){
...
bool
BToPhiMuMu::isGenPhiCharged(const reco::Candidate *p){
...
bool
BToPhiMuMu::isGenKshort(const reco::Candidate *p){
...
bool
BToPhiMuMu::isGenMuonP(const reco::Candidate *p){
...
void 
BToPhiMuMu::saveKshortVariables(RefCountedKinematicTree ksVertexFitTree, 
...
void 
BToPhiMuMu::saveSoftMuonVariables(pat::Muon iMuonM, pat::Muon iMuonP, 
...
void 
BToPhiMuMu::saveDimuVariables(double DCAmumBS, double DCAmumBSErr, 
...
void 
BToPhiMuMu::saveMuonTriggerMatches(const pat::Muon iMuonM, const pat::Muon iMuonP)
{	
...
void
BToPhiMuMu::saveTruthMatch(const edm::Event& iEvent){
  double deltaEtaPhi; 
